 ...
 */
package de.jwiegmann.bookstore.domain.builder;
...
public final class ObjectBuilder<T> {
  ...
  public static <V> ObjectBuilder<V> fromGenericType(
          final Class<?> subclass, final Class<?> superclass) {
    return fromGenericType(subclass, superclass, 0);
  }

  public static <V> ObjectBuilder<V> fromGenericType(
          final Class<?> subclass, final Class<?> superclass, final int parameterIndex) {
  ...
  private Constructor<T> resolveConstructor(
          final Object[] arguments, final boolean convertParameters) {
  ...
  private Constructor<T> resolveConstructor(
          final Class<?>[] parameterTypes, final boolean convertParameters) {
    ...
    if (ambiguousConstructor != null) {
      String msg =
              String.format(
                      "More that one constructor found for parameter types %s. Found %s and %s",
  ...
  private boolean matches(
    ...
    for (int i = 0; i < parameterTypes.length; i++) {
      ...
      if (parameterTypes[i] != null
              && !constructorParameterTypes[i].isAssignableFrom(parameterTypes[i])) {
        if (!convertParameterTypes
                || !isConvertible(parameterTypes[i], constructorParameterTypes[i])) {
          return false;
        }
      }
    }
  ...
  private boolean isMoreSpecific(
          final Constructor<T> constructor, final Constructor<T> resolvedConstructor) {
  ...
  private Object[] convertArguments(final Constructor<T> constructor, final Object... arguments)
    ...
    for (int i = 0; i < arguments.length; i++) {
      ...
      if (!parameterTypes[i].isInstance(arguments[i])) {
        Constructor<?> convertingConstructor =
                parameterTypes[i].getDeclaredConstructor(arguments[i].getClass());
  ...
  private void validate(final Constructor<T> constructor, final Object... arguments) {
    if (validator != null) {
      Set<ConstraintViolation<T>> violations =
              validator.forExecutables().validateConstructorParameters(constructor, arguments);
